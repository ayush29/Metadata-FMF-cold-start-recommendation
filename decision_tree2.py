...
def splitUsers(data, feature_index):
    # Get the indices for the when the rating is greater than 3: LIKE
    indices_like = np.where(data[:, feature_index] > 0.3)[0]

    # Get the indices for the when the rating is less than 3: DISLIKE
    indices_dislike = np.where((data[:, feature_index] <= 0.3) & (data[:, feature_index] != 0))[0]
...
class Tree:
	...
	def __init__(self, root_node, rating_matrix, K, max_depth = 5):
		self.root = root_node
	...
	def traverse(self, user_answers):
		...
		while current_node.like != None or current_node.dislike != None or current_node.unknown != None : 
			if user_answers[current_node.feature_index] == 0:
				current_node = current_node.like
			elif user_answers[current_node.feature_index] == 1:
...
#                print ("zzz", current_node.user_vector.shape)
	...
	def getUserVectors(self, features_matrix, K):
		ultimate_user_vector = np.zeros((len(features_matrix), K))
		
		for i in range(len(features_matrix)):
			# Stores the user response
			user_response = np.zeros(len(features_matrix[0]))
			
			# Get the responses using the feature matrix
			for j in range(len(features_matrix[0])):
				if features_matrix[i][j] > 0.3:
					user_response[j] = 0
				elif features_matrix[i][j] == 0:
					user_response[j] = 2
				else:
					user_response[j] = 1
	...
	def fitTree(self, current_node, rating_matrix,user_features_matrix, movie_vectors, K):
		...
		split_values = np.zeros(len(user_features_matrix[0]))

		# print "Error Before: ", error_before

		for feature_index in range(len(user_features_matrix[0])):
			# Split the rating_matrix into like, dislike and unknown
			(indices_like, indices_dislike, indices_unknown) = splitUsers(user_features_matrix, feature_index)
			...
			# Store the split criteria values for current Feature_index
			split_values[feature_index]  = value

		# Get the index of the Feature with the maximum split value
		...
		# Split the rating_matrix into like, dislike and unknown
		(indices_like, indices_dislike, indices_unknown) = splitUsers(user_features_matrix, bestFeature)
		...
		if error_before - split_values[bestFeature] > 10**(-3):
			...
			if len(like) != 0:
				self.fitTree(current_node.like, like,user_features_matrix[indices_like], movie_vectors, K)

			current_node.dislike = Node(current_node, current_node.depth + 1)
			current_node.dislike.user_vector = dislike_vector
			if len(dislike) != 0:
				self.fitTree(current_node.dislike, dislike,user_features_matrix[indices_dislike], movie_vectors, K)
			
			current_node.unknown = Node(current_node, current_node.depth + 1)
			current_node.unknown.user_vector = unknown_vector
			if len(unknown) != 0:
				self.fitTree(current_node.unknown, unknown,user_features_matrix[indices_unknown], movie_vectors, K)
